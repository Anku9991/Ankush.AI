...
export default async function InstagramFeed() {
  ...
  try {
    const cleanToken = rawToken ? rawToken.trim().replace(/^["']|["']$/g, '') : '';
    ...
    if (!isFBToken) {
      ...
      if (fbPagesData.data && fbPagesData.data.length > 0) {
        ...
        if (igAccountData.instagram_business_account) {
          ...
          console.error(`Graph API: Facebook Page (ID: ${pageId}) is NOT linked to an Instagram Business account.`);
        }
      } else {
        console.error("Graph API: Failed to fetch Facebook Pages. Invalid token or missing permissions.");
      }
    }
  } catch (error) {
